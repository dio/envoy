 ...
 */
class SkywalkingAccessLogStreamer {
public:
  virtual ~SkywalkingAccessLogStreamer() {}
   ...
   */
  virtual void send(ServiceMeshMetric& message, const std::string& log_name) PURE;
};

typedef std::shared_ptr<SkywalkingAccessLogStreamer> SkywalkingAccessLogStreamerSharedPtr;

/**
 * Production implementation of SkywalkingAccessLogStreamer that supports per-thread and per-log
 * streams.
 */
class SkywalkingAccessLogStreamerImpl : public Singleton::Instance,
                                        public SkywalkingAccessLogStreamer {
public:
  SkywalkingAccessLogStreamerImpl(Grpc::AsyncClientFactoryPtr&& factory,
                                  ThreadLocal::SlotAllocator& tls,
                                  const LocalInfo::LocalInfo& local_info);

  // GrpcAccessLogStreamer
  void send(ServiceMeshMetric& message, const std::string& log_name) override {
...
private:
  ...
  struct ThreadLocalStreamer : public ThreadLocal::ThreadLocalObject {
    ...
    void send(ServiceMeshMetric& message, const std::string& log_name);
 ...
 */
class SkywalkingAccessLog : public AccessLog::Instance {
public:
  SkywalkingAccessLog(AccessLog::FilterPtr&& filter,
...
private:
  ...
  SkywalkingAccessLogStreamerSharedPtr skywalking_access_log_streamer_;
  ...
  const Http::LowerCaseString source_header_{"x-skywalking-source"};
};
