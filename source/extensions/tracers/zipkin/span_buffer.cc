...
JsonV2Serializer::toListOfSpans(const Span& zipkin_span, Utility::Replacements& replacements) const {
  ...
  for (const auto& annotation : zipkin_span.annotations()) {
    ...
    if (zipkin_span.isSetDuration()) {
      // Since SPAN_DURATION has the same data type with SPAN_TIMESTAMP, we use Utility::uint64Value to
      // store it.
