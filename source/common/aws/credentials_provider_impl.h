...
class MetadataCredentialsProviderBase : public CredentialsProvider,
                                        public Logger::Loggable<Logger::Id::aws> {
public:
  MetadataCredentialsProviderBase(Api::Api& api, MetadataFetcherPtr&& fetcher)
      : api_(api), fetcher_(std::move(fetcher)) {}
...
public:
  InstanceProfileCredentialsProvider(Api::Api& api, MetadataFetcherPtr&& fetcher)
      : MetadataCredentialsProviderBase(api, std::move(fetcher)) {}
...
public:
  TaskRoleCredentialsProvider(
      Api::Api& api, MetadataFetcherPtr&& fetcher, const std::string& credential_uri,
      const absl::optional<std::string>& authorization_token = absl::optional<std::string>())
      : MetadataCredentialsProviderBase(api, std::move(fetcher)), credential_uri_(credential_uri),
...
public:
  ...
  createInstanceProfileCredentialsProvider(Api::Api& api, MetadataFetcherPtr&& fetcher) const PURE;
};

class DefaultCredentialsProviderChain : public CredentialsProviderChain,
                                        public CredentialsProviderChainFactories {
public:
  DefaultCredentialsProviderChain(Api::Api& api) : DefaultCredentialsProviderChain(api, *this) {}
...
private:
  ...
  CredentialsProviderSharedPtr createTaskRoleCredentialsProvider(
    ...
    return std::make_shared<TaskRoleCredentialsProvider>(api, std::move(fetcher), credential_uri,
                                                         authorization_token);
  }

  CredentialsProviderSharedPtr
  createInstanceProfileCredentialsProvider(Api::Api& api,
                                           MetadataFetcherPtr&& fetcher) const override {
    return std::make_shared<InstanceProfileCredentialsProvider>(api, std::move(fetcher));
  }
};
