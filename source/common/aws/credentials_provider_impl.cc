...
bool InstanceProfileCredentialsProvider::needsRefresh() {
  if (api_.timeSource().systemTime() - last_updated_ > REFRESH_INTERVAL) {
    return true;
  }
  return false;
}
...
void InstanceProfileCredentialsProvider::refresh() {
  auto dispatcher = api_.allocateDispatcher();
...
bool TaskRoleCredentialsProvider::needsRefresh() {
  ...
  if (expiration_time_ - api_.timeSource().systemTime() < REFRESH_GRACE_PERIOD) {
    return true;
  }
  return false;
}
...
DefaultCredentialsProviderChain::DefaultCredentialsProviderChain(
    Api::Api& api, const CredentialsProviderChainFactories& factories) {
  ...
  if (relative_uri != nullptr) {
    ...
    add(factories.createTaskRoleCredentialsProvider(api, factories.createMetadataFetcher(), uri,
    ...
    if (authorization_token != nullptr) {
      ENVOY_LOG(debug, "Using task role credentials provider with URI: {} and authorization token",
                full_uri);
      add(factories.createTaskRoleCredentialsProvider(api, factories.createMetadataFetcher(),
                                                      full_uri, authorization_token));
    } else {
      ENVOY_LOG(debug, "Using task role credentials provider with URI: {}", full_uri);
      add(factories.createTaskRoleCredentialsProvider(api, factories.createMetadataFetcher(),
                                                      full_uri, absl::optional<std::string>()));
    }
    ...
    add(factories.createInstanceProfileCredentialsProvider(api, factories.createMetadataFetcher()));
  }
}

MetadataFetcherPtr createMetadataFetcher(Api::Api& api);
