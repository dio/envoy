...
LogicalDnsCluster::LogicalDnsCluster(const envoy::api::v2::Cluster& cluster,
  ...
  const auto& socket_address =
      endpoints.size() == 1
          ? (endpoints[0].has_endpoint() ? endpoints[0].endpoint().address().socket_address()
                                         : endpoints[0].address().socket_address())
          : hosts[0].socket_address();
  dns_url_ = fmt::format("tcp://{}:{}", socket_address.address(), socket_address.port_value());
  health_check_config_ =
      endpoints[0].has_endpoint()
          ? endpoints[0].endpoint().health_check_config()
