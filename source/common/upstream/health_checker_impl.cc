...
void HttpHealthCheckerImpl::HttpActiveHealthCheckSession::onInterval() {
  ...
  for (const auto& header_entry : parent_.headers_to_add_) {
    const std::string& value = header_entry.second->value();
    if (header_entry.second->append()) {
      request_headers.addReferenceKey(header_entry.first, value);
    } else {
      request_headers.setReferenceKey(header_entry.first, value);
    }
  }
