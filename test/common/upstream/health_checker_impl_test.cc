...
public:
  ...
  void setupServiceNameWithOptionalCustomHostHC(const absl::optional<std::string>& host) {
    auto config = createGrpcHealthCheckConfig();
    config.mutable_grpc_health_check()->set_service_name("service");
    if (host.has_value()) {
      config.mutable_grpc_health_check()->set_host(host.value());
    }
    health_checker_.reset(new TestGrpcHealthCheckerImpl(*cluster_, config, dispatcher_, runtime_,
                                                        random_,
                                                        HealthCheckEventLoggerPtr(event_logger_)));
  ...
  void expectHealthCheckWithOptionalCustomHost(const absl::optional<std::string>& host) {
    setupServiceNameWithOptionalCustomHostHC(host);
    ...
    EXPECT_CALL(test_sessions_[0]->request_encoder_, encodeHeaders(_, false))
        .WillOnce(Invoke([&](const Http::HeaderMap& headers, bool) {
          ...
          EXPECT_EQ(host.has_value() ? host.value() : "fake_cluster",
                    headers.Host()->value().c_str());
        }));
...
TEST_F(GrpcHealthCheckerImplTest, Success) {
  expectHealthCheckWithOptionalCustomHost(absl::nullopt);
}

// Test single host check success with host header is overridden.
TEST_F(GrpcHealthCheckerImplTest, SuccessWithCustomHost) {
  expectHealthCheckWithOptionalCustomHost("grpc.service");
}
...
TEST_F(GrpcHealthCheckerImplTest, SuccessResponseSplitBetweenChunks) {
  setupServiceNameWithOptionalCustomHostHC(absl::nullopt);
